...
<div class="mb-3">
    ...
    <select id="company_id" name="company_id" class="form-select @error('company_id') is-invalid @enderror" required>
        ...
        @foreach ($companies as $company)
            <option value="{{ $company->id }}" {{ (int) old('company_id', $location?->company_id) === $company->id ? 'selected' : '' }}>
                {{ $company->name }}
            </option>
        @endforeach
    </select>
...
<div class="mb-3">
    ...
    <select id="branch_id" name="branch_id" class="form-select @error('branch_id') is-invalid @enderror">
        ...
        @foreach ($branches as $branch)
            <option value="{{ $branch->id }}" {{ (int) old('branch_id', $location?->branch_id) === $branch->id ? 'selected' : '' }}>
...
<div class="mb-3">
    ...
    <textarea id="address" name="address" rows="2"
              class="form-control @error('address') is-invalid @enderror">{{ old('address', $location?->address) }}</textarea>
    @error('address')
        <div class="invalid-feedback">{{ $message }}</div>
    @enderror
</div>
