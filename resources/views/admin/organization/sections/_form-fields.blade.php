...
<div class="mb-3">
    ...
    <select id="company_id" name="company_id" class="form-select @error('company_id') is-invalid @enderror" required>
        ...
        @foreach ($companies as $company)
            <option value="{{ $company->id }}" {{ (int) old('company_id', $section?->company_id) === $company->id ? 'selected' : '' }}>
                {{ $company->name }}
            </option>
        @endforeach
    </select>
...
<div class="mb-3">
    ...
    <select id="department_id" name="department_id" class="form-select @error('department_id') is-invalid @enderror">
        ...
        @foreach ($departments as $department)
            <option value="{{ $department->id }}" {{ (int) old('department_id', $section?->department_id) === $department->id ? 'selected' : '' }}>
...
<div class="row">
    ...
    <div class="col-12 col-md-6 mb-3">
        ...
        <input id="code" type="text" name="code" value="{{ old('code', $section?->code) }}"
